function createBubbleChart(error, countries, continentNames) {
  var age = countries.map(function(country) { return +country.age; });
  var meanPopulation = d3.mean(age),
      ageExtent = d3.extent(age),
      AgeScaleX,
      AgeScaleY;

  var lesions = d3.set(countries.map(function(country) { return country.lesionclass; }));
  var LesionColorScale = d3.scaleOrdinal(d3.schemeCategory10)
        .domain(lesions.values());
  ...
  function toggleLesionKey(showLesionKey) {
    var keyElementWidth = 150,
        keyElementHeight = 30;
    var onScreenYOffset = keyElementHeight*1.5,
        offScreenYOffset = 100;

    if (d3.select(".lesion-key").empty()) {
      createLesionKey();
    }
    var lesionKey = d3.select(".lesion-key");

    if (showLesionKey) {
      translateLesionKey("translate(0," + (height - onScreenYOffset) + ")");
    } else {
      translateLesionKey("translate(0," + (height + offScreenYOffset) + ")");
    }

    function createLesionKey() {
      var keyWidth = keyElementWidth * lesions.values().length;
      var lesionKeyScale = d3.scaleBand()
        .domain(lesions.values())
        .range([(width - keyWidth) / 2, (width + keyWidth) / 2]);

      svg.append("g")
        .attr("class", "lesion-key")
        .attr("transform", "translate(0," + (height + offScreenYOffset) + ")")
        .selectAll("g")
        .data(lesions.values())
        .enter()
          .append("g")
            .attr("class", "lesion-key-element");

      d3.selectAll("g.lesion-key-element")
        .append("rect")
          .attr("width", keyElementWidth)
          .attr("height", keyElementHeight)
          .attr("x", function(d) { return lesionKeyScale(d); })
          .attr("fill", function(d) { return LesionColorScale(d); });

      d3.selectAll("g.lesion-key-element")
        .append("text")
          .attr("text-anchor", "middle")
          .attr("x", function(d) { return lesionKeyScale(d) + keyElementWidth/2; })
          .text(function(d) { return continentNames[d]; });

      // The text BBox has non-zero values only after rendering
      d3.selectAll("g.lesion-key-element text")
          .attr("y", function(d) {
    ...
    function translateLesionKey(translation) {
      lesionKey
        .transition()
        .duration(500)
  ...
  function flagFill() { // just for image stuff
  ...
  function createCircles() {
    ...
    function updateCountryInfo(country) {
      var info = "";
      if (country) {
        info = [country.CountryName, formatAge(country.Population)].join(": ");
      }
  ...
  function updateCircles() {
    circles
      .attr("fill", function(d) {
        return flagFill() ? "url(#" + d.CountryCode + ")" : LesionColorScale(d.ContinentCode);
      });
  }
  ...
  function createForces() {
    ...
    function createPopulationForces() {
      var continentNamesDomain = lesions.values().map(function(lesionclass) {
        return continentNames[lesionclass];
      });
      var scaledPopulationMargin = circleSize.max;

      AgeScaleX = d3.scaleBand()
        .domain(continentNamesDomain)
        .range([scaledPopulationMargin, width - scaledPopulationMargin*2]);
      AgeScaleY = d3.scaleLog()
        .domain(ageExtent)
        .range([height - scaledPopulationMargin, scaledPopulationMargin*2]);

      var centerCirclesInScaleBandOffset = AgeScaleX.bandwidth() / 2;
      return {
        x: d3.forceX(function(d) {
            return AgeScaleX(continentNames[d.ContinentCode]) + centerCirclesInScaleBandOffset;
          }).strength(forceStrength),
        y: d3.forceY(function(d) {
          return AgeScaleY(d.Population);
        }).strength(forceStrength)
      };
    }
  ...
  function addFillListener() {
    d3.selectAll('input[name="fill"]')
      .on("change", function() {
        toggleLesionKey(!flagFill() && !populationGrouping());
        updateCircles();
      });
  }
  ...
  function addGroupingListeners() {
    ...
    function togglePopulationAxes(showAxes) {
      ...
      function createAxes() {
        ...
        var xAxis = d3.axisBottom(AgeScaleX)
        ...
        var yAxis = d3.axisLeft(AgeScaleY)
          .ticks(numberOfTicks, tickFormat);
        svg.append("g")
          .attr("class", "y-axis")
